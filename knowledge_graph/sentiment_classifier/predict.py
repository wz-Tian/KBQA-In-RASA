...
def main():
    ...
    data = pd.read_csv("D:\huadi\project\code\knowledge_graph\graph_build\movieInfo2.csv", encoding='utf-8', dtype=str)

    result = []
    for row in range(data.shape[0]):

        comment = data.loc[row, 'info']
        result.append(str(predict(comment, model, device, word2ix)))

    data.insert(loc=len(data.columns), column='sentiment', value=result)
    data.to_csv("D:\huadi\project\code\knowledge_graph\sentiment_classifier\\newMovieInfo.csv", index=None, encoding='utf-8', quoting=1)
